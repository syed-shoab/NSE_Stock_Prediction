...
def loadDataset():
    ...
    dataFrame = yf.download("AAPL", start=start, end=end)
    ...
    df = yf.download(['AAPL', 'GE', 'GOOG', 'IBM', 'MSFT'], start=start, end=end, auto_adjust=False)
    dfcomp = df.xs('Adj Close', axis=1, level=0)
...
def dfcorr():
    ...
    text.insert(END, "Correlation from Apple Competitor Stock\n\n")
...
def dataPreProcess():
    ...
    forecast_col = 'Close'  # Use 'Close' consistently
    
    dfreg['label'] = dfreg[forecast_col].shift(-forecast_out)

    dfreg.dropna(inplace=True)

    dfreg["Volume"] = dfreg["Volume"] / 1e6
    scaler = RobustScaler() 
    X = scaler.fit_transform(dfreg.drop(['label'], axis=1))
    ...
    text.insert(END, "Number of Test Samples : " + str(len(X_test)) + "\n")
...
def predModel():
    ...
    x_pred = np.array(test.drop(['Unnamed: 0'], axis=1))
